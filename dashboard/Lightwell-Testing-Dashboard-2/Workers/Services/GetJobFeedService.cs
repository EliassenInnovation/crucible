...
{
    public class GetJobFeedService : BackgroundService
    {
        readonly TestResultWorker _testResultWorker;
        readonly IConfiguration _config;

        public GetJobFeedService(TestResultWorker testResultWorker,IConfiguration config)
        {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    await Task.Delay(TimeSpan.FromMinutes(_config.GetValue<int>("FailedJobFeedRefreshInterval")), stoppingToken); // Run every minute
                }
                catch (Exception ex)
                {
